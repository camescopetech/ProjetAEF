...
def minimal_automaton(automaton) : #based on find_cycles
        ...
        for letter in alphabet : #look for new states to find other paths
            ...
            if  next_state : 
                if next_state[0] not in path : #state not already in the current path
                    ...
                    stack.append((next_state[0], path + next_state)) # supply stack with updated values
                else : maybe.append((next_state[0],path)) # we are in a loop : the states are usefull if we get back to an usefull element without loop
        stack.pop(0)
    ...
    df_good =  df[df.index.isin(usefull_states)]  #delete useless state
    df_good.loc[:,alphabet] = df_good[alphabet].applymap(lambda x: q1.null_transition if x not in usefull_states else x)  #delete transitions to useless states  
    return  df_good         


# print(q1.get_good_type(test.auto3,'dataFrame'))
# print(minimal_automaton(test.auto3))
